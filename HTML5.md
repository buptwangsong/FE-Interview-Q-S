...
4. [javascript】谈谈HTML5: Web-Worker、canvas、indexedDB、拖拽事件](https://www.cnblogs.com/penghuwan/p/7492814.html)
...
> javascript是单线程的，所以，持续时间较长的计算会阻塞渲染线程，进而导致无法再文本框中输入文本，单击按钮等等。该问题的解决方案是`web workers`可以让web 应用程序具备后台处理能力，对多线程的支持非常友好。

> `web worker`机制能够让你创建一个在后台线程运行的脚本，该脚本不会对当前正在执行的脚本产生任何影响。

> `web worker`中执行的脚本不能访问该页面的`window`对象，也就是说`web worker`不能访问当前的web页面以及DOM API。虽然`web worker`不会导致当前页面的UI停止响应，但是仍会消耗CPU周期，导致系统反应速度变慢。

> 关于该技术的具体应用，参考这里 [javascript】谈谈HTML5: Web-Worker、canvas、indexedDB、拖拽事件](https://www.cnblogs.com/penghuwan/p/7492814.html)
